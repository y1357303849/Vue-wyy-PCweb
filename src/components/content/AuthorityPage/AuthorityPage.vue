<template>
  <div class="authorityBox">
    ...
    <div class="detailTopBox">
      <!-- 歌单图片 -->
      <div class="musicImg">
        <img :src="atDetail.coverImgUrl" alt="" />
      </div>
      <!-- 歌单信息区域 -->
      <div class="detailMessageBox">
        ...
        <div class="listNameBox">
          ...
          <div class="listName">{{ atDetail.name }}</div>
        </div>
        <!-- 创建者头像、昵称、时间 -->
        <div class="userBox">
          <div class="creator">
            <img :src="atDetail.creator.avatarUrl" alt="" />
          </div>
          <div class="userName">{{ atDetail.creator.nickname }}</div>
        ...
        <div class="listButton">
          ...
          <div class="collectBtn">
            ...
            <div>收藏({{ atDetail.subscribedCount }})</div>
          </div>
          <div class="shareBtn">
            <i class="iconfont icon-fenxiang"></i>
            <div>分享({{ atDetail.shareCount }})</div>
          </div>
        </div>
        <!-- 标签 -->
        <div class="tag">
          标签：<a href="#" v-for="(item, index) in atDetail.tags" :key="index"
            >{{ item }}/</a
          >
        </div>
        <!-- 简介 -->
        <div class="intro">
          <p class="intro1">简介：</p>
          <p class="intro2">{{ atDetail.description }}</p>
        </div>
      </div>
      <!-- 歌曲数、播放数 -->
      <div class="numBox">
        <div class="singNum">
          歌曲数
          <p>{{ atDetail.trackCount }}</p>
        </div>
        <div class="playerNum">
          播放数
          <p>{{ atDetail.playCount }}</p>
        </div>
      </div>
    </div>
    <!-- 歌单主体区域 -->
    <div class="detailContentBox">
      ...
      <div class="listContent" v-show="showType === 1">
        ...
        <div class="songLists">
          <ul>
            <li v-for="(item, index) in atDetail.tracks" :key="index">
...
export default {
  ...
  mounted() {
    // 接收父页面点击传送过来的数据id
    bus.$on("authorityData", (val) => {
      this.atDataId = val;
      console.log(this.atDataId);
      // 将数据存储到localStorage里
      window.localStorage.setItem(
        "authorityData",
        JSON.stringify(this.atDataId)
      );
    });
  },
  methods: {
    ...
    async playListDetail() {
      ...
      console.log(musicId.authorityData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/detail?id=" + musicId.authorityData,
        {
          params: {
            playlist: this.atDetail,
          },
        }
      );
      this.atDetail = res.playlist;
      console.log(this.atDetail);
    },
    // 请求对应的歌单评论数据
    async commentDetail() {
      // 将存储的数据赋值到commentId2里
      const commentId2 = window.localStorage;
      // console.log(commentId2.authorityData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/comment/playlist?id=" + commentId2.authorityData,
        {
          params: {
    ...
    async collectorDetail() {
      // 将存储的数据赋值到collectorId2里
      const collectorId2 = window.localStorage;
      // console.log(collectorId2 .authorityData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/subscribers?id=" + collectorId2.authorityData + "&limit=30",
        {
          params: {
