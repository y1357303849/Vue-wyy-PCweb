<template>
  <div class="globalBox">
    ...
    <div class="detailTopBox">
      <!-- 歌单图片 -->
      <div class="musicImg">
        <img :src="globalDetail.coverImgUrl" alt="" />
      </div>
      <!-- 歌单信息区域 -->
      <div class="detailMessageBox">
        ...
        <div class="listNameBox">
          ...
          <div class="listName">{{ globalDetail.name }}</div>
        </div>
        <!-- 创建者头像、昵称、时间 -->
        <div class="userBox">
          <div class="creator">
            <img :src="globalDetail.creator.avatarUrl" alt="" />
          </div>
          <div class="userName">{{ globalDetail.creator.nickname }}</div>
        ...
        <div class="listButton">
          ...
          <div class="collectBtn">
            ...
            <div>收藏({{ globalDetail.subscribedCount }})</div>
          </div>
          <div class="shareBtn">
            <i class="iconfont icon-fenxiang"></i>
            <div>分享({{ globalDetail.shareCount }})</div>
          </div>
        </div>
        <!-- 标签 -->
        <div class="tag">
          标签：<a
            href="#"
            v-for="(item, index) in globalDetail.tags"
            :key="index"
            >{{ item }}/</a
          >
        </div>
        ...
        <div class="intro">
          <p class="intro1">简介：</p>
          <p class="intro2">{{ globalDetail.description }}</p>
        </div>
      </div>
      <!-- 歌曲数、播放数 -->
      <div class="numBox">
        <div class="singNum">
          歌曲数
          <p>{{ globalDetail.trackCount }}</p>
        </div>
        <div class="playerNum">
          播放数
          <p>{{ globalDetail.playCount }}</p>
        </div>
      </div>
    </div>
    <!-- 歌单主体区域 -->
    <div class="detailContentBox">
      ...
      <div class="listContent" v-show="showType === 1">
        ...
        <div class="songLists">
          <ul>
            <li v-for="(item, index) in globalDetail.tracks" :key="index">
      ...
      <div class="commentContent" v-show="showType === 2">
        ...
        <div class="brilliantBox">
          ...
          <div class="brillantList">
            <ul>
              <li v-for="(item, index) in glComment" :key="index">
                <div class="userImg">
      ...
      <div class="collectorContent" v-show="showType === 3">
        <ul>
          <li v-for="(item, index) in glCollector" :key="index">
...
export default {
  ...
  mounted() {
    ...
    bus.$on("globalData", (val) => {
      ...
      // 将数据存储到localStorage里
      window.localStorage.setItem("globalData", JSON.stringify(this.globalId));
    });
  },
  methods: {
    // 请求对应的歌单数据
    async playListDetail() {
      // 将存储的数据赋值到musicId4里
      const musicId4 = window.localStorage;
      console.log(musicId4.globalData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/detail?id=" + musicId4.globalData,
        {
          params: {
            playlist: this.globalDetail,
          },
        }
      );
      this.globalDetail = res.playlist;
      console.log(this.globalDetail);
    },
    // 请求对应的歌单评论数据
    async commentDetail() {
      // 将存储的数据赋值到commentId6里
      const commentId6 = window.localStorage;
      // console.log(commentId6.globalData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/comment/playlist?id=" + commentId6.globalData,
        {
          params: {
            comments: this.glComment,
          },
        }
      );
      this.glComment = res.comments;
      // console.log(this.glComment[0].user.avatarUrl);
    },
    // 请求对应的歌单收藏人数据
    async collectorDetail() {
      // 将存储的数据赋值到collectorId6里
      const collectorId6 = window.localStorage;
      // console.log(collectorId6 .globalData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/subscribers?id=" + collectorId6.globalData + "&limit=30",
        {
          params: {
            collector: this.glCollector,
          },
        }
      );
      this.glCollector = res.subscribers;
      console.log(this.glCollector.avatarUrl);
    },
