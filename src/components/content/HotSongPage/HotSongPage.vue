<template>
  <div class="hotSongBox">
    ...
    <div class="detailTopBox">
      <!-- 歌单图片 -->
      <div class="musicImg">
        <img :src="hotSongDetail.coverImgUrl" alt="" />
      </div>
      <!-- 歌单信息区域 -->
      <div class="detailMessageBox">
        ...
        <div class="listNameBox">
          ...
          <div class="listName">{{ hotSongDetail.name }}</div>
        </div>
        <!-- 创建者头像、昵称、时间 -->
        <div class="userBox">
          <div class="creator">
            <img :src="hotSongDetail.creator.avatarUrl" alt="" />
          </div>
          <div class="userName">{{ hotSongDetail.creator.nickname }}</div>
        ...
        <div class="listButton">
          ...
          <div class="collectBtn">
            ...
            <div>收藏({{ hotSongDetail.subscribedCount }})</div>
          </div>
          <div class="shareBtn">
            <i class="iconfont icon-fenxiang"></i>
            <div>分享({{ hotSongDetail.shareCount }})</div>
          </div>
        </div>
        <!-- 标签 -->
        <div class="tag">
          标签：<a
            href="#"
            v-for="(item, index) in hotSongDetail.tags"
            :key="index"
            >{{ item }}/</a
          >
        </div>
        ...
        <div class="intro">
          <p class="intro1">简介：</p>
          <p class="intro2">{{ hotSongDetail.description }}</p>
        </div>
      </div>
      <!-- 歌曲数、播放数 -->
      <div class="numBox">
        <div class="singNum">
          歌曲数
          <p>{{ hotSongDetail.trackCount }}</p>
        </div>
        <div class="playerNum">
          播放数
          <p>{{ hotSongDetail.playCount }}</p>
        </div>
      </div>
    </div>
    <!-- 歌单主体区域 -->
    <div class="detailContentBox">
      ...
      <div class="listContent" v-show="showType === 1">
        ...
        <div class="songLists">
          <ul>
            <li v-for="(item, index) in hotSongDetail.tracks" :key="index">
      ...
      <div class="collectorContent" v-show="showType === 3">
        <ul>
          <li v-for="(item, index) in hotCollector" :key="index">
...
export default {
  ...
  mounted() {
    // 接收父页面点击传送过来的数据id
    bus.$on("hotSongData", (val) => {
      this.hotSongId = val;
      console.log(this.hotSongId);
      // 将数据存储到localStorage里
      window.localStorage.setItem(
        "hotSongData",
        JSON.stringify(this.hotSongId)
      );
    });
  },
  methods: {
    ...
    async playListDetail() {
      // 将存储的数据赋值到musicId3里
      const musicId3 = window.localStorage;
      console.log(musicId3.hotSongData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/detail?id=" + musicId3.hotSongData,
        {
          params: {
            playlist: this.hotSongDetail,
          },
        }
      );
      this.hotSongDetail = res.playlist;
      console.log(this.hotSongDetail);
    },
    // 请求对应的歌单评论数据
    async commentDetail() {
      // 将存储的数据赋值到commentId5里
      const commentId5 = window.localStorage;
      // console.log(commentId5.hotSongData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/comment/playlist?id=" + commentId5.hotSongData,
        {
          params: {
            comments: this.hotComment,
          },
        }
      );
      this.hotComment = res.comments;
      // console.log(this.hotComment[0].user.avatarUrl);
    },
    // 请求对应的歌单收藏人数据
    async collectorDetail() {
      // 将存储的数据赋值到collectorId5里
      const collectorId5 = window.localStorage;
      // console.log(collectorId5 .hotSongData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/subscribers?id=" + collectorId5.hotSongData + "&limit=30",
        {
          params: {
            collector: this.hotCollector,
          },
        }
      );
      this.hotCollector = res.subscribers;
      console.log(this.hotCollector.avatarUrl);
    },
