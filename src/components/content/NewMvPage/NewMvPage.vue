    ...
    <div class="rightContent">
      ...
      <div class="mvIntroBox">
        ...
        <div class="timeAndPlay">
          <div class="releaseTime">发布时间：{{ newDetail.publishTime }}</div>
          <div class="watchNum">播放次数：{{ newDetail.playCount }}次</div>
        </div>
        <div class="introText">简介：{{ newDetail.desc }}</div>
      </div>
      <!-- 相关推荐 -->
      <div class="moreMvBox">
...
export default {
  ...
  mounted() {
    // 接收父页面点击传送过来的数据id
    bus.$on("newMvData", (val) => {
      this.newDataId = val.id;
      console.log(this.newDataId);
      // 将数据存储到localStorage里
      window.localStorage.setItem("newMvData", JSON.stringify(this.newDataId));
    });
  },
  methods: {
    // 请求对应的MV数据
    async getMvDetail() {
      ...
      // console.log(onlyMvId.newMvData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应独家MV详情页的数据
        "/mv/detail?mvid=" + onlyMvId.newMvData,
        {
          params: {
            res: this.newDetail,
          },
    ...
    async getMvVideo() {
      ...
      // console.log(mvVideoId.newMvData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应独家Mv视频
        "/mv/url?id=" + mvVideoId.newMvData,
        {
          params: {
            res: this.mvVideo,
          },
