<template>
  <div class="newSingBox">
    <!-- 歌单头部区域 -->
    <div class="detailTopBox">
      <!-- 歌单图片 -->
      <div class="musicImg">
        <img :src="newSingDetail.coverImgUrl" alt="" />
      </div>
      <!-- 歌单信息区域 -->
      <div class="detailMessageBox">
        ...
        <div class="listNameBox">
          ...
          <div class="listName">{{ newSingDetail.name }}</div>
        </div>
        <!-- 创建者头像、昵称、时间 -->
        <div class="userBox">
          <div class="creator">
            <img :src="newSingDetail.creator.avatarUrl" alt="" />
          </div>
          <div class="userName">{{ newSingDetail.creator.nickname }}</div>
        ...
        <div class="listButton">
          ...
          <div class="collectBtn">
            ...
            <div>收藏({{ newSingDetail.subscribedCount }})</div>
          </div>
          <div class="shareBtn">
            <i class="iconfont icon-fenxiang"></i>
            <div>分享({{ newSingDetail.shareCount }})</div>
          </div>
        </div>
        <!-- 标签 -->
        <div class="tag">
          标签：<a
            href="#"
            v-for="(item, index) in newSingDetail.tags"
            :key="index"
            >{{ item }}/</a
          >
        </div>
        ...
        <div class="intro">
          <p class="intro1">简介：</p>
          <p class="intro2">{{ newSingDetail.description }}</p>
        </div>
      </div>
      <!-- 歌曲数、播放数 -->
      <div class="numBox">
        <div class="singNum">
          歌曲数
          <p>{{ newSingDetail.trackCount }}</p>
        </div>
        <div class="playerNum">
          播放数
          <p>{{ newSingDetail.playCount }}</p>
        </div>
      </div>
    </div>
    <!-- 歌单主体区域 -->
    <div class="detailContentBox">
      ...
      <div class="listContent" v-show="showType === 1">
        ...
        <div class="songLists">
          <ul>
            <li v-for="(item, index) in newSingDetail.tracks" :key="index">
      ...
      <div class="collectorContent" v-show="showType === 3">
        <ul>
          <li v-for="(item, index) in newCollector" :key="index">
...
export default {
  ...
  mounted() {
    // 接收父页面点击传送过来的数据id
    bus.$on("newSingData", (val) => {
      this.newSingId = val;
      console.log(this.newSingId);
      // 将数据存储到localStorage里
      window.localStorage.setItem(
        "newSingData",
        JSON.stringify(this.newSingId)
      );
    });
  },
  methods: {
    ...
    async playListDetail() {
      // 将存储的数据赋值到musicId1里
      const musicId1 = window.localStorage;
      console.log(musicId1.newSingData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/detail?id=" + musicId1.newSingData,
        {
          params: {
            playlist: this.newSingDetail,
          },
        }
      );
      this.newSingDetail = res.playlist;
      console.log(this.newSingDetail);
    },
    // 请求对应的歌单评论数据
    async commentDetail() {
      // 将存储的数据赋值到commentId3里
      const commentId3 = window.localStorage;
      // console.log(commentId3.newSingData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/comment/playlist?id=" + commentId3.newSingData,
        {
          params: {
      ...
      // console.log(this.newComment[0].user.avatarUrl);
    },
    // 请求对应的歌单收藏人数据
    async collectorDetail() {
      // 将存储的数据赋值到collectorId3里
      const collectorId3 = window.localStorage;
      // console.log(collectorId3 .newSingData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/subscribers?id=" + collectorId3.newSingData + "&limit=30",
        {
          params: {
      ...
      this.newCollector = res.subscribers;
      console.log(this.newCollector.avatarUrl);
    },
