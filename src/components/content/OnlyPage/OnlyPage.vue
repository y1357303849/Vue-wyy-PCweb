    ...
    <div class="rightContent">
      ...
      <div class="moreMvBox">
        ...
        <div class="moreMvList">
          <ul>
            <li v-for="(item, index) in mvMore" :key="index">
              <div class="mvImg">
...
export default {
  ...
  mounted() {
    // 接收父页面点击传送过来的数据id
    bus.$on("onlyData", (val) => {
      this.onlyDataId = val.id;
      console.log(this.onlyDataId);
      // 将数据存储到localStorage里
      window.localStorage.setItem("onlyData", JSON.stringify(this.onlyDataId));
    });
  },
  methods: {
    // 请求对应的MV数据
    async getMvDetail() {
      ...
      // console.log(onlyMvId.onlyData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应独家MV详情页的数据
        "/mv/detail?mvid=" + onlyMvId.onlyData,
        {
          params: {
            res: this.onlyMvDetail,
          },
        }
      );
      this.onlyMvDetail = res.data;
      // console.log(this.onlyMvDetail);
    },
    // 请求对应的Mv视频
    async getMvVideo() {
      // 将存储的数据赋值到onlyMvId里
      const mvVideoId = window.localStorage;
      // console.log(mvVideoId.onlyData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应独家Mv视频
        "/mv/url?id=" + mvVideoId.onlyData,
        {
          params: {
            res: this.mvVideo,
          },
    ...
    async getMvComment() {
      ...
      // console.log(mvCommentId.onlyData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应独家MV详情页的数据
        "/comment/mv?id=" + mvCommentId.onlyData,
        {
          params: {
            comments: this.mvComment,
          },
        }
      );
    ...
    async getMvMore() {
      ...
      // console.log(mvMoreId.onlyData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应独家MV详情页的数据
        "/simi/mv?mvid=" + mvMoreId.onlyData,
        {
          params: {
            mvMore: this.mvMore,
          },
