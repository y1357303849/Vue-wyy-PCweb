<template>
  <div class="originalBox">
    ...
    <div class="detailTopBox">
      <!-- 歌单图片 -->
      <div class="musicImg">
        <img :src="orDetail.coverImgUrl" alt="" />
      </div>
      <!-- 歌单信息区域 -->
      <div class="detailMessageBox">
        ...
        <div class="listNameBox">
          ...
          <div class="listName">{{ orDetail.name }}</div>
        </div>
        <!-- 创建者头像、昵称、时间 -->
        <div class="userBox">
          <div class="creator">
            <img :src="orDetail.creator.avatarUrl" alt="" />
          </div>
          <div class="userName">{{ orDetail.creator.nickname }}</div>
        ...
        <div class="listButton">
          ...
          <div class="collectBtn">
            ...
            <div>收藏({{ orDetail.subscribedCount }})</div>
          </div>
          <div class="shareBtn">
            <i class="iconfont icon-fenxiang"></i>
            <div>分享({{ orDetail.shareCount }})</div>
          </div>
        </div>
        <!-- 标签 -->
        <div class="tag">
          标签：<a href="#" v-for="(item, index) in orDetail.tags" :key="index"
            >{{ item }}/</a
          >
        </div>
        <!-- 简介 -->
        <div class="intro">
          <p class="intro1">简介：</p>
          <p class="intro2">{{ orDetail.description }}</p>
        </div>
      </div>
      <!-- 歌曲数、播放数 -->
      <div class="numBox">
        <div class="singNum">
          歌曲数
          <p>{{ orDetail.trackCount }}</p>
        </div>
        <div class="playerNum">
          播放数
          <p>{{ orDetail.playCount }}</p>
        </div>
      </div>
    </div>
    <!-- 歌单主体区域 -->
    <div class="detailContentBox">
      ...
      <div class="listContent" v-show="showType === 1">
        ...
        <div class="songLists">
          <ul>
            <li v-for="(item, index) in orDetail.tracks" :key="index">
      ...
      <div class="commentContent" v-show="showType === 2">
        ...
        <div class="brilliantBox">
          ...
          <div class="brillantList">
            <ul>
              <li v-for="(item, index) in orComment" :key="index">
                <div class="userImg">
      ...
      <div class="collectorContent" v-show="showType === 3">
        <ul>
          <li v-for="(item, index) in orCollector" :key="index">
...
export default {
  ...
  mounted() {
    // 接收父页面点击传送过来的数据id
    bus.$on("originalData", (val) => {
      this.originalId = val;
      console.log(this.originalId);
      // 将数据存储到localStorage里
      window.localStorage.setItem(
        "originalData",
        JSON.stringify(this.originalId)
      );
    });
  },
  methods: {
    ...
    async playListDetail() {
      // 将存储的数据赋值到musicId2里
      const musicId2 = window.localStorage;
      console.log(musicId2.originalData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/detail?id=" + musicId2.originalData,
        {
          params: {
            playlist: this.orDetail,
          },
        }
      );
      this.orDetail = res.playlist;
      console.log(this.orDetail);
    },
    // 请求对应的歌单评论数据
    async commentDetail() {
      // 将存储的数据赋值到commentId4里
      const commentId4 = window.localStorage;
      // console.log(commentId4.originalData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/comment/playlist?id=" + commentId4.originalData,
        {
          params: {
            comments: this.orComment,
          },
        }
      );
      this.orComment = res.comments;
      // console.log(this.orComment[0].user.avatarUrl);
    },
    // 请求对应的歌单收藏人数据
    async collectorDetail() {
      // 将存储的数据赋值到collectorId4里
      const collectorId4 = window.localStorage;
      // console.log(collectorId4 .originalData);
      const { data: res } = await request.get(
        // 网络请求尾部加上当前点击的组件对应的id，以此来获取对应歌单详情页的数据
        "/playlist/subscribers?id=" + collectorId4.originalData + "&limit=30",
        {
          params: {
            collector: this.orCollector,
          },
        }
      );
      this.orCollector = res.subscribers;
      console.log(this.orCollector.avatarUrl);
    },
